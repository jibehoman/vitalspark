...
<nl>
    ...
    D Kibble writes: &quot;Connections in general…. Would it not be possible to abstract in the first instance (simple mode vs expert mode) the JMS connection entirely from the ESB containers? We know that we associate an ESB container with a message broker cluster and we know what QoS we are wishing to use. Could Sonic not then transparently create the connections and pools targeted to the associated broker cluster(s)?  This would greatly simplify the config element for basic users and a lot of use cases. Currently we have lots of connection variations per broker cluster to try to optimise utilisation based on volume and QoS, that ends up being a config management headache and a lot of updating when new services / requirements / volume are added
    <br><br>
